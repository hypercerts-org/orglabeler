...
3. Review and merge **Release**. The release workflow listens for that merged pull request, and publishes only when its head repository is this repository and its head ref is exactly `changeset-release/main`. Before tagging anything, it checks out and validates the exact merge commit, then runs `npm test`, `npx tsc --noEmit`, and `npm run build`.
